    ...
    def init(self, configfile=None, *args, **kwargs):
        if configfile:
            try:
                ...
                print("Can't open the specified configuration file {0}, error: {1}".format(configfile, e))
                return
    ...
    def load(self, pargs=None, extras=None, comments=True, *args, **kwargs):
        ...
        if pargs:
            for key in pargs.__dict__:
                ...
                value = pargs.__dict__[key]
                if not (key in self.config and not value):
